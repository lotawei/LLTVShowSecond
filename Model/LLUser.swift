...
//扩展一个归档
extension   LLUser{
    //   必须拿到objid  string  才能 去数据库存储
    typealias ObjectidBlock = (_ objid:String) -> Void
    ...
    //bmob 的objid
    func   userobjid(_ objresult:@escaping ObjectidBlock){
        let   query = BmobQuery(className: "_User")
        query?.whereKey("username", equalTo: username)
        query?.findObjectsInBackground({ (users, err) in
