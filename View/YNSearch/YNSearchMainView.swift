...
open class YNSearchMainView: UIView {
    ...
    open func initView(categories: [String]) {
        self.categoryLabel = UILabel(frame: CGRect(x: margin, y: 0, width: bwidth - 40, height: 50))
        self.categoryLabel.text = "热门"
        ...
        self.searchHistoryLabel = UILabel(frame: CGRect(x: margin, y: originY + 30, width: bwidth - 40, height: 40))
        self.searchHistoryLabel.text = "搜索历史"
        self.searchHistoryLabel.font = UIFont.systemFont(ofSize: 14)
    ...
    open func redrawSearchHistoryButtons() {
        ...
        let histories = ynSearch.getSearchRecentHistories(6) ?? [String]()
